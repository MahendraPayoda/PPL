...
class CustomRadioDialog extends StatefulWidget {
  ...
  const CustomRadioDialog(this.value, {super.key, this.onPressed});
...
class _CustomRadioDialogState extends State<CustomRadioDialog> {
  RadioOptionModel? selectedOption;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      updateValue();
      //  widget.value.page = 1;
      //  widget.value.employeeNameList.clear();
      // onLoading();
    });
  ...
  List<RadioOptionModel> filteredOptions = [];
  ...
  onLoading() async {
    var newData = await widget.value.getEmployeeData();
    if (newData.isNotEmpty) {
      widget.value.employeeNameListForCaptain.addAll(newData);
      widget.value.page++;
      setState(() {});
    }
  ...
  updateValue() {
    setState(() {
      widget.value.employeeNameListForCaptain.clear();
      widget.value.employeeNameListForCaptain.addAll(widget.value.tempEmployeeNameListForCaptain);
      if (widget.value.employeeNameListForCaptain.isNotEmpty) {
        if (widget.value.selectedViceCaptainId != 0) {
          widget.value.employeeNameListForCaptain
              ...
              Expanded(
                child: RawScrollbar(
                  ...
                  child: SmartRefresher(
                    ...
                    controller: widget.value.refreshController,
                    ...
                    child: ListView(
                      ...
                      ListView.builder(
                       physics: const NeverScrollableScrollPhysics(),
                        ...
                        shrinkWrap: true,
                        itemCount: widget.value.employeeNameListForCaptain.length,
                        itemBuilder: (context, index) {
                          return InkWell(
                            onTap: () {
                              widget.value.selectedCaptainId =
                                  widget.value.employeeNameListForCaptain[index].employeeID!;
                              widget.value.captainName = widget.value.employeeNameListForCaptain[index];
                              setState(() {});
                            },
                            child: Padding(
                              padding: const EdgeInsets.symmetric(vertical: 5),
                              child: Row(children: [
                                ...
                                const SizedBox(width: 10),
                                Flexible(
                                  child: Text(widget.value.employeeNameListForCaptain[index].employeeName ?? '',
                                      overflow: TextOverflow.ellipsis,
                                      style: textTheme().bodyMedium?.copyWith(
                                          color: AppColors.titleColor,
                                          fontSize: 13,
...
class RadioOptionModel {
  ...
  RadioOptionModel({required this.name, this.value = false});
}
